 ...
 * 相机View
 */
public class ZKCameraTextureView extends TextureView implements Camera.AutoFocusCallback {
    ...
    private Camera.PictureCallback mPictureCallback;//照片回调
    ...
    public static final int previewWidth = 1920;//1920
    public static final int previewHeight = 1080;//1080
    public static final int pictureWidth = 1920;
    public static final int pictureHeight = 1080;
    ...
    public static boolean isPreviewing = false;//是否在预览

    public ZKCameraTextureView(Context context) {
        super(context);
        init(context, null);
    }

    public ZKCameraTextureView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private SurfaceTextureListener mSurfaceTextureListener = new SurfaceTextureListener() {
        ...
        public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
            try {
                ...
                e.printStackTrace();
                Toast.makeText(getContext(), getResources().getString(R.string.camera), Toast.LENGTH_SHORT).show();
            }
            Matrix mirrorMatrix = new Matrix();
            mirrorMatrix.setScale(-1, 1, getWidth() / 2, 0);
            ZKCameraTextureView.this.setTransform(mirrorMatrix);
    ...
    private void initParameters() {
        ...
        try {
            ...
            for (Camera.Size size1 : size) {
                Logger.i("openAndroidCamera: width=" + size1.width + " height=" + size1.height);
            }
    ...
    private void initPreviewSize() {
        ...
        Size fitPreviewSize = mCamera.new Size(previewWidth, previewHeight); // 获取适合的预览大小
        ...
        Logger.d("initPreviewSize() mPreviewWidth: " + mPreviewWidth + ", mPreviewHeight: " + mPreviewHeight);
    ...
    private Size getFitPreviewSize(Parameters parameters) {
        ...
        for (int i = 0; i < previewSizes.size(); i++) {
            ...
            Logger.d("SupportedPreviewSize, width: " + previewSize.width + ", height: " + previewSize.height);
    ...
    public void initPreviewBuffer() {
        ...
        // 先获取像素格式字节数 * 预览摄像头宽度 * 预览摄像头高度 / 8 （转成 byte）
        int bufferSize = ImageFormat.getBitsPerPixel(ImageFormat.NV21) * mPreviewWidth * mPreviewHeight / 8;
        mBuffer = new byte[bufferSize]; // 初始化预览缓冲数据的大小
        Logger.d("initPreviewBuffer() mBuffer.length: " + mBuffer.length);
    ...
    private float getScreenScale() {
        ...
        Logger.d("displayMetrics.widthPixels : " + width);
        Logger.d("displayMetrics.heightPixels : " + height);
    ...
    private void focusOnPoint(int x, int y) {
        ...
        if (parameters.getMaxNumFocusAreas() > 0) {
            ...
            Logger.d("focus area (" + left + ", " + top + ", " + right + ", " + bottom + ")");
...
//        File mediaStorageDir = new File(FileUtils.avatarPath);
...
//    }
    ...
    private void resetMatrix() {
        int width = getWidth();
        int height = getHeight();
        Logger.d("resetMatrix() width: " + width + ", height: " + height);
        if (width > height) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Logger.d("originalWidth : " + originalWidth);
        Logger.d("originalHeight : " + originalHeight);
        Logger.d("finalWidth : " + finalWidth);
        Logger.d("finalHeight : " + finalHeight);
        setMeasuredDimension(finalWidth, finalHeight);
//        setMeasuredDimension(2133, 1200);
    }
